    ...
    try {
        ...
        if (type === 'license' && itemId) {
            ...
            console.log(`License granted + Cashback ${cashback} Pi to transferable balance.`);
             ...
             // Energy Logic
             if (itemId === 'energy_small') user.energyBalance += 20; // Changed to match frontend data (20 in shopContent, 50 in shopProducts? check consistency)
             else if (itemId === 'energy_medium') user.energyBalance += 120;
             else if (itemId === 'energy_large') user.energyBalance += 250; 
             else if (itemId === 'energy_full') user.energyBalance = 100; // Reset to full or add 100? Let's add 100 to be safe
