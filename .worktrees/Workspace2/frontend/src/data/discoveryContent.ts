...
export function enrichDiscoveryLayer(layerId: string, lang: string = 'fr'): DiscoveryContent | null {
    ...
    const tString = (field: string | LocalizedContent | undefined): string => {
        if (!field) return '';
        if (typeof field === 'string') return field;
        return (lang === 'en' ? field.en : field.fr) || field.fr || '';
    };

    const tArray = (field: string[] | LocalizedArray | undefined): string[] => {
        if (!field) return [];
    ...
    return {
        ...data,
        title: tString(data.title),
        description: tString(data.description),
        content: tString(data.content),
        highlights: tArray(data.highlights)
    };
}
