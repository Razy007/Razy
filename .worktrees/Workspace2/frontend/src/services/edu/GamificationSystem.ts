...
export const ACHIEVEMENTS: Achievement[] = [
    // ========== PROGRESSION ==========
    {
        ...
        xpReward: 50,
        piReward: 0.0001,
        condition: (p) => p.layerProgress.length > 0,
        unlocked: false
    },
    {
        id: 'knowledge_seeker',
        ...
        piReward: 0.0005,
        condition: (p) => {
            const completedCourseIds = new Set(p.layerProgress.filter(lp => lp.mastered).map(lp => lp.courseId));
            return completedCourseIds.size >= 5;
        },
        unlocked: false
    },
    {
    ...
    // ========== SÉCURITÉ ==========
    {
        ...
        piReward: 0.0004,
        condition: (p) => {
            const securityQuestions = (p.questionHistory || [])
                .filter(entry => entry.questionId.includes('scam') || entry.questionId.includes('security'))
