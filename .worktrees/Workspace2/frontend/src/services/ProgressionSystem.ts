...
export class ProgressionSystem {
    ...
    static isCourseUnlocked(course: Course, userProgress: UserProgress): UnlockStatus {
        // Premium courses require premium status (handled here)
        if (course.premium || course.isPremium) {
             const hasPremium = userProgress.purchaseHistory?.includes('premium_pass') || 
                                userProgress.inventory?.some(i => i.itemId === 'license_pro') || 
                                false;
             if (!hasPremium) {
                return {
                    isUnlocked: false,
                    reason: '💎 Contenu Premium - Débloquez-le dans la boutique !'
                };
             }
        }
        ...
        // 🔓 MASTER KEY: Users with Level >= 10 bypass strict sequential order
        // This allows experts to explore freely while guiding beginners
        const isMaster = userProgress.level >= 10;
        
        if (!isMaster && course.requiredCourses && course.requiredCourses.length > 0) {
    ...
    static calculateCourseProgress(course: Course, userProgress: UserProgress): number {
        const completedLayers = userProgress.layerProgress.filter(
            lp => lp.courseId === course.id && lp.mastered
        );
    ...
    static organizeCoursesByCategory(
        ...
        const categorized = new Map<string, { course: Course; unlockStatus: UnlockStatus; progress: number }[]>();
