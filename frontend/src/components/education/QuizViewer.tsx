...
export const QuizViewer: React.FC<QuizViewerProps> = ({ layer, userProgress, onComplete, onClose }) => {
    ...
    if (showResults) {
        ...
        // Handle Watch Ad for Free Retry
        const handleWatchAd = async () => {
            try {
                const success = await AdManager.showRewardedAd({
                    type: 'RETRY',
                    amount: 1,
                    meta: { layerId: layer.id }
                });

                if (success) {
                    toast.success(t('ads.reward_granted', 'Récompense reçue ! Retry gratuit.'));
                    // Reset quiz without consuming energy/XP
                    setHasStarted(false);
                ...
                toast.error(t('ads.error', 'Erreur lors de l\'affichage de la publicité.'));
            }
        };
