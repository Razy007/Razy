...
export class EnergySystem {
  ...
  static getCurrentEnergy(energyState: EnergyState): EnergyState {
    const now = Date.now();
    const timeSinceRecharge = now - energyState.lastRecharge;
    const hoursElapsed = timeSinceRecharge / (1000 * 60 * 60);
    
    // Calculate recharged energy
    const rechargedAmount = Math.floor(hoursElapsed * energyState.rechargeRate);
    const newCurrent = Math.min(
      energyState.current + rechargedAmount,
      energyState.max
    );

    // Check for rest bonus (daily login after 8+ hours)
    let restBonus = 0;
    if (timeSinceRecharge >= this.REST_THRESHOLD && energyState.current < energyState.max) {
      restBonus = this.REST_BONUS;
    }

    return {
      ...energyState,
      current: Math.min(newCurrent + restBonus, energyState.max),
      lastRecharge: rechargedAmount > 0 ? now : energyState.lastRecharge,
      restBonus: restBonus
    };
  }
  ...
  static timeUntilFullRecharge(energyState: EnergyState): number {
    ...
    const hoursNeeded = energyNeeded / currentState.rechargeRate;
