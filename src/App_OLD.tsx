...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-black text-white pb-20 font-sans">
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-[9999] flex items-center justify-center p-4" onClick={() => setShowProfile(false)}>
          <div className="bg-gradient-to-br from-purple-900 to-indigo-900 rounded-2xl p-6 max-w-md w-full" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4 mt-4">
              <p className="text-red-300 font-bold mb-2 text-sm flex items-center gap-2">
                <Shield size={16} />
                Zone de Test / Vérification
              </p>
              <p className="text-gray-400 text-xs mb-3">Utilisez ces boutons pour simuler les différents profils et vérifier les accès.</p>
              <div className="grid grid-cols-2 gap-2">
                <button
                  onClick={() => {
                    setAuthStatus('guest');
                    setKycStatus('none');
                    alert("Mode Invité activé");
                  }}
                  className={`text-xs p-2 rounded border transition ${authStatus === 'guest' ? 'bg-white text-black border-white' : 'text-white border-white/20 hover:bg-white/10'}`}
                >
                  Mode Invité
                </button>
                <button
                  onClick={() => {
                    setAuthStatus('pioneer');
                    setKycStatus('none');
                    alert("Mode Pioneer (Non-KYC) activé");
                  }}
                  className={`text-xs p-2 rounded border transition ${authStatus === 'pioneer' && kycStatus !== 'verified' ? 'bg-white text-black border-white' : 'text-white border-white/20 hover:bg-white/10'}`}
                >
                  Pioneer (Non-KYC)
                </button>
                <button
                  onClick={() => {
                    setAuthStatus('pioneer');
                    setKycStatus('verified');
                    alert("✅ Mode KYC Vérifié activé! Accès complet débloqué.");
                  }}
                  className={`col-span-2 text-xs p-2 rounded border transition ${kycStatus === 'verified' ? 'bg-green-500 text-black font-bold border-green-500' : 'text-green-400 border-green-500/30 hover:bg-green-500/20'}`}
                >
                  ✅ Simuler KYC Vérifié
                </button>
              </div>
      ...
      {/* Referral Test Component (Development Only) */}
