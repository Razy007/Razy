...
Write-Host "DEPLOIEMENT PRODUCTION - Academy of Pi (No Sudo)" -ForegroundColor Cyan
Write-Host "======================================================`n" -ForegroundColor Cyan
...
# Deploiement (sans sudo - suppose que pioneer a les permissions)
...
ssh -i "$keyPath" "${remoteUser}@${serverIp}" 'if [ -d /var/www/pioneer-academy ]; then mv /var/www/pioneer-academy /var/www/pioneer-academy_backup; fi'

# Creation repertoire
Write-Host "   Creation du repertoire..." -ForegroundColor Yellow
ssh -i "$keyPath" "${remoteUser}@${serverIp}" 'mkdir -p /var/www/pioneer-academy'

# Decompression
Write-Host "   Decompression..." -ForegroundColor Yellow
ssh -i "$keyPath" "${remoteUser}@${serverIp}" 'unzip -o /tmp/dist_deploy.zip -d /var/www/pioneer-academy'
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "Note: Si erreur de permission, l'utilisateur pioneer doit avoir acces a /var/www/`n" -ForegroundColor Yellow
    exit 1
}
